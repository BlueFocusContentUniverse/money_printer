...
class ChatTTSAudioService:
    def __init__(self,audio_output_dir):
        super().__init__()
        ...
        if st.session_state.get('use_random_voice'):
            if 'audio_seed' not in st.session_state:
                st.session_state['audio_seed'] = random_with_system_time()
                print("不存在audio_seed 根据系统时间随机生成",st.session_state.get('audio_seed'))
    ...
    def chat_with_content(self, content, audio_output_file):
        ...
        try:
            ...
            with zipfile.ZipFile(BytesIO(response.content), "r") as zip_ref:
                zip_ref.extractall(self.audio_output_dir)
                file_names = zip_ref.namelist()
                output_file = os.path.join(self.audio_output_dir, file_names[0])
