...
def merge_get_video_list():
    ...
    if video_text_list is not None:
        video_scene_text_list = video_text_list
...
def get_video_scene_video_list(video_dir_list):
    ...
    if video_dir_list and video_dir_list[0] is not None:
        for video_file in os.listdir(video_dir_list[0]):
            if video_file.lower().endswith(('.mp4', '.mov')):
                video_scene_video_list.append(os.path.join(video_dir_list[0], video_file))
