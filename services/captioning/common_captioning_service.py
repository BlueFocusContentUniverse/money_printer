...
class Captioning(object):
    ...
    def captions_from_offline_results(self) -> List[caption_helper.Caption]:
        print("准备调用 get_captions 函数")
        print("语言:", self._user_config["language"])
        print("最大行宽:", self._user_config["max_line_length"])
        print("最大行数:", self._user_config["lines"])
        print("结果列表:", self._offline_results)
        captions = caption_helper.get_captions(self._user_config["language"], self._user_config["max_line_length"],
                                               self._user_config["lines"], self._offline_results)
        # Save the last caption.
        #print(self._offline_results)
        print("CAPTION0-------------------------------",captions)
