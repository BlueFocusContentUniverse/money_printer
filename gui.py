...
from common.config.config import my_config, save_config, languages, test_config, local_audio_tts_providers, \
    ...
    chattts_enhanced_style_options, chattts_enhanced_name_options,\
...
def set_ui_language():
    logger.info('Setting UI language to: %s', st.session_state['ui_language'])
    my_config['ui']['language'] = st.session_state['ui_language'].split(" - ")[0].strip()
    logger.info('UI language set to: %s', my_config['ui']['language'])
    save_config()
...
def get_ChatTTS_Enhanced_server_location():
    return my_config['audio'].get('local_tts', {}).get('ChatTTS_Enhanced', {}).get('server_location', '')

def set_ChatTTS_Enhanced_server_location():
    #return my_config['audio'].get('local_tts', {}).get('ChatTTS_Enhanced', {}).get('server_location', '')
    test_config(my_config, "audio", "local_tts", 'ChatTTS_Enhanced', 'server_location')
    my_config['audio']['local_tts']['ChatTTS_Enhanced']['server_location'] = st.session_state['ChatTTS_Enhanced_server_location']
    save_config()
...
with audio_container:
    ...
    with local_tts_container:
        ...
        if local_audio_tts_provider == 'ChatTTS_Enhanced':
                        st.text_input(label=tr("ChatTTS_Enhanced http server location"),
                          placeholder=tr("Input ChatTTS_Enhanced http server address"),
                          value=get_ChatTTS_Enhanced_server_location(),
                          key="ChatTTS_Enhanced_server_location", on_change=set_ChatTTS_Enhanced_server_location)
